...
var Product = function () {
  ...
  Product.prototype.Init = function () {
    ...
    a.setAttribute("onclick", "WantBuy(this.id, this.inStock)");
    a.innerHTML = "Купить";
...
var FeltBoots = function (_super) {
  ...
  FeltBoots.prototype.CalculateFlags = function () {
    ...
    if (this.list != null) for (var i = 0; i < this.list.length; i++) {
    ...
    this.haveColors = [];
    if (this.list != null) for (var i = 0; i < this.list.length; i++) {
      if (this.haveColors.indexOf(this.list[i].color) == -1) this.haveColors[k++] = this.list[i].color;
    }
  };
...
function CheckWireless(flag) {
  ...
  if (flag) {
    for (var i = 0; i < this.productList.length; i++) {
      if (productList[i] instanceof Headphones && productList[i].isWireless) productList[i].Init();
    }
  } else {
    for (var i = 0; i < this.productList.length; i++) {
      productList[i].Init();
...
function CheckHardDiskHDD(flag) {
  ...
  if (flag) {
    for (var i = 0; i < this.productList.length; i++) {
      if (productList[i] instanceof Computers && productList[i].hardDisk == "HDD") productList[i].Init();
    }
  } else {
    for (var i = 0; i < this.productList.length; i++) {
      productList[i].Init();
...
function CheckHardDiskSSD(flag) {
  ...
  if (flag) {
    for (var i = 0; i < this.productList.length; i++) {
      if (productList[i] instanceof Computers && productList[i].hardDisk == "SSD") productList[i].Init();
    }
  } else {
    for (var i = 0; i < this.productList.length; i++) {
      productList[i].Init();
...
function CheckGamers(flag) {
  ...
  if (flag) {
    for (var i = 0; i < this.productList.length; i++) {
      if (productList[i] instanceof Computers && productList[i].isGamers) productList[i].Init();
    }
  } else {
    for (var i = 0; i < this.productList.length; i++) {
      productList[i].Init();
...
function CheckMale(flag) {
  ...
  if (flag) {
    for (var i = 0; i < this.productList.length; i++) {
      if (productList[i] instanceof Clothes && productList[i].isMale) productList[i].Init();
    }
  } else {
    for (var i = 0; i < this.productList.length; i++) {
      productList[i].Init();
...
function CheckFemale(flag) {
  ...
  if (flag) {
    for (var i = 0; i < this.productList.length; i++) {
      if (productList[i] instanceof Clothes && productList[i].isFemale) productList[i].Init();
    }
  } else {
    for (var i = 0; i < this.productList.length; i++) {
      productList[i].Init();
...
var Basket = function () {
  ...
  Basket.prototype.Add = function (val) {
    ...
    if (isNaN(num) || !((num ^ 0) === num) || num == 0 || productList[val].inStock < num) {
      if (productList[val].inStock < num) document.getElementById('modlalMessag').innerHTML = "Столько на складе нет";else document.getElementById('modlalMessag').innerHTML = "Введите целое число";
      return false;
    } else {
...
function WantBuy(val, tov) {
  console.log(tov);
  document.getElementById('modlalBtn').setAttribute("value", val);
  var p = document.getElementById('number__tovar');
}

var basket = new Basket();
var productList = [new Headphones(0, "Наушники фирмы1", 816, "Прекрасные наушники! Сама английская королева слушает жесткий металл через такие же!", 4, true), new FeltBoots(1, "Валенки2", 91.2, "Хороший выбор! В них тепло, хорошо. Обувь многосезонная - лето, осень, зима, весна.", 6, [{
  dimension: 44,
  color: Color.Black,
  quantity: 2
}, {
  ...
  color: Color.Gray,
  quantity: 1
}]), new Headphones(2, "Наушники фирмы4", 119.50, "Дёшево не значит плохо! Эти наушники стоят своих денег!", 30, false), new Headphones(3, "Наушники фирмы2", 144, "Это оптимальный выбор! Налетай торопись!", 15, true), new Balalaika(4, "Балалайка1", 915, "Сам страдивари её выстругал! Мастер Страдивари Аарон Моисеевич ©. В комплекте к балалайке должен идти медведь.", 1), new FeltBoots(5, "Валенки3", 65, "Валенки знаменитой российской фабрики Красный ЦинБаоЧен. Оригинальный продукт сделаный по технологиям прошлого.", 1), new Headphones(6, "Наушники фирмы3", 265, "Тру поклонники музыки обязательно такие имеют! А ты что? Ты не тру?!", 0), new FeltBoots(7, "Валенки1", 666.66, "Валенки великолепной работы слепого мастера Игната! В комплекте к валенкам идёт кокошник.", 2, [{
  dimension: 45,
  color: Color.Pink,
  quantity: 1
}, {
  ...
  color: Color.Pink,
  quantity: 1
}]), new Balalaika(8, "Балалайка2", 217, "Обычная балалайка белорусской фирмы Змрочныя мелодыі.", 1), new Computers(9, "Ноутбук Lenovo", 25000, "Прекрасный ноутбук для игр!", 10, "SSD", true), new Computers(10, "Ноутбук Asus", 10000, "Прекрасный ноутбук для офиса!", 8, "HDD", false), new Clothes(11, "Пальто", 1500, "Теплое зимнее пальто", 5, true, false), new Clothes(11, "Куртка", 950, "Осенняя", 7, false, true)];